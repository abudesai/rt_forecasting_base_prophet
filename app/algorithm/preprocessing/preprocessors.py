...
class HistoryFieldsRelabeler(BaseEstimator, TransformerMixin):
    ''' creates a dummy feature if the feature doesn't exist. This is used to add the target
    feature for the prediction task. '''

    def __init__(self, epoch_field, target_field, prediction_field):
         self.epoch_field = epoch_field
         self.target_field = target_field
         self.prediction_field = prediction_field
         self.prophet_epoch_field = "ds"
         self.prophet_target_field = "y"
         self.prophet_prediction_field = "yhat"

    def fit(self, X, y=None): return self

    def transform(self, X):
        X.rename(columns={
            self.epoch_field: self.prophet_epoch_field,
            self.target_field: self.prophet_target_field
            }, inplace=True)
        return X  
    
    def inverse_transform(self, preds): 
        preds.rename(columns={
            self.prophet_epoch_field: self.epoch_field,
            self.prophet_target_field: self.target_field,
            self.prophet_prediction_field: self.prediction_field,
...
class SpEventsFieldsRelabeler(BaseEstimator, TransformerMixin):
    ''' creates a dummy feature if the feature doesn't exist. This is used to add the target
    feature for the prediction task. '''

    def __init__(self, event_field, epoch_field, lower_window_field, upper_window_field ):
         self.event_field = event_field
         self.epoch_field = epoch_field
         self.lower_window_field = lower_window_field
         self.upper_window_field = upper_window_field
         
         self.prophet_event_field = "holiday"
         self.prophet_epoch_field = "ds"
         self.prophet_lower_window_field = "lower_window"
         self.prophet_upper_window_field = "upper_window"

    def fit(self, X, y=None): return self

    def transform(self, X):
        X.rename(columns={
            self.event_field: self.prophet_event_field,
            self.epoch_field: self.prophet_epoch_field,
            self.lower_window_field: self.prophet_lower_window_field,
            self.upper_window_field: self.prophet_upper_window_field,
            }, inplace=True)
        return X    
...
class MissingValueTagger(BaseEstimator, TransformerMixin):  
    ...
    def transform(self, data):  
        ...
        data.loc[idx, self.missing_field] = 1.   
        
        # fill the missing values with a very low value. 
        # we set it to be observed_min - observed_range from non-missing values
